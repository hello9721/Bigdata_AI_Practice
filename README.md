...
:point_right: [PACKAGE.R](https://github.com/hello9721/R_Practice/blob/main/PACKAGE.R) :point_left:
               ...
               => 수업 과정 중 활용한 R 패키지들 모음 ( 샘플데이터활용 / 함수활용 / 시각화활용 )
#
#### 22/08/24  
...
#### 22/11/18
    Day 23 1 - 실습 [ 병렬 처리 ]
           2 - RNN ( 순환 신경망 ) - SimpleRNN 모델
           
#
#### 22/11/28
    Day 25 1 - PyQt5 [ 레이아웃 ]
           2 - PyQt5 [ 웹 크롤링을 이용한 테이블 입력 ]

#
#### 22/12/01
    Day 26 1 - PyQt5 [ Day 25 프로그램 마무리 - 테이블 입력된 데이터를 통한 그래프 표시 ]
           2 - PyQt5 [ Day 25 프로그램을 응용하여 실습 ]
           3 - 배열 [ 1차원 배열 & 2차원 배열 초기화 ]
           4 - CNN 모델의 저장과 로드
#
#### 22/12/02
    Day 27 1 - PyQt5 [ File Dialog 기능 사용하여 이미지 파일 불러와서 표시 ]
           2 - PyQt5 [ MariaDB 와 연동하여 데이터 추가 기능 작성 및 그래프 표시 ]
           3 - PyQt5 [ Day 26 에서 저장된 모델을 사용하여 이미지 파일을 불러와 종류를 분류하는 프로그램 만들기 ]
#
#### 22/12/15 ~
    Project - Day 1 - Practice [ 본 데이터로 프로젝트를 진행하기 전에 더 작은 양을 데이터로 미리 연습 ]
              Day 2 - Practice [ 미리 훈련시킨 모델로 예측값을 뽑아낸 후 원본 값과 비교하여 Qt 로 그래프 표시 ]
              Day 3 - Project  [ 과거 데이터를 토대로 모델을 학습 시켜 예측값을 뽑아낸 후 원본 값과 비교 ]
                      Project  [ 과거 데이터를 원본 데이터 DB로 넣고 1분에 한 행씩 데이터를 뽑아 프로젝트 DB에
                                 추가해주는 프로그램 구현 하여 직접 센서에서 데이터를 받는 것 같은 환경을 만듦 ]
                      Project - sample [ 모델의 성능과 업데이트 프로그램 동작 확인을 위해 Practice에서 만들었던 Qt 프로그램을 수정하여
                                         1분에 한번씩 모든 데이터를 업데이트 하면서 예측 데이터를 그려주도록 작성 ]
              Day 4 - Project [ 전체 데이터를 학습시킨 모델로 교체 및 샘플 UI 작성 (DB연결 / 그래프 / 테이블 작성) ]
              Day 5 - Project [ 샘플 UI를 정리하여 메인 UI 작성 및 기능 추가 (과거 이벤트 조회창과 기간 지정하여 데이터 조회 작성 및 연결)
              Day 6 - Project [ 스타일시트 적용하여 디자인 점검 및 모델 연결하여 예측값 그래프 연결 ]
